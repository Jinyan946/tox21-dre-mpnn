...
def optimization_manager(config):
    ...
    def optimize_fn(
        ...
        if step > 0 and step % config.training.snapshot_freq == 0:
            for g in optimizer.param_groups:
                g["lr"] = (
                    0.5 * lr * (1.0 + np.cos((step / config.training.n_iters) * np.pi))
                )
...
#             sde, score_model, train=train, continuous=continuous
#         )
...
#         with torch.enable_grad():
#             t.requires_grad_()
#             xt_score_dt = autograd.grad(
#                 score_fn(xt, t)[-1].sum(), t, create_graph=True
#             )[0]
...
def get_time_sde_loss_fn_flow_z_interpolate(
    flow,
    flow_name,
    sde,
    train,
                ...
                if flow_name in ["mintnet", "nice", "realnvp"]:
                    ...
                    if "noise" in flow_name or "copula" in flow_name:
                        ...
                        z_batch = flow.module.transform_to_noise(
                            z_batch, transform=True, train=train
                        )
                    else:
            ...
            with torch.no_grad():
                if flow_name in ["mintnet", "nice", "realnvp"]:
                    ...
                    if "noise" in flow_name or "copula" in flow_name:
                        ...
                        xt = flow.module.sample(
                            zt.view(n, -1), context=None, rescale=True
                        )
                        px = flow.module.sample(
                            px.view(n, -1), context=None, rescale=True
                        )
        else:
            xt = zt
            # px = px
...
# def get_time_prob_path_loss_fn_flow_z_interpolate(
#     flow,
#     flow_name,
#     prob_path,
#     train,
...
#     def ctsm_loss_fn(score_t, epsilon, qx, t):
#         targets = prob_path.noise_pred_target(epsilon, qx, t, factor)

#         ctsm_loss = torch.square(targets - score_t)
#         return ctsm_loss
...
#           loss: A scalar that represents the average loss value across the mini-batch.
#         """
#         score_fn = mutils.get_c_time_score_fn(
#             prob_path, score_model, train=train, continuous=continuous
#         )
...
#         if iw and not interpolate:
#             with torch.no_grad():
#                 t = torch.sort(t)[0]  # HACK

#         #   # TODO: trying this out!
#         #   u0 = torch.rand(1, device=device) * (sde.T - eps) + eps
#         #   t = (u0 + torch.arange(1, n + 1).to(device) / n) % 1

#         epsilon = torch.randn_like(batch)  # noise
#         mean, std = prob_path.marginal_prob(z_batch, t)  # feed in z into SDE
#         zt = epsilon * std[:, None, None, None] + mean
...
#                 else:
#                     xt = flow.module.sample(
#                         zt.view(batch_size, -1), context=None, rescale=True
#                     )
...
#             xt = xt.view(batch_size, -1)  # mlp

#         unweighted_loss = ctsm_loss_fn(score_fn(xt, t), epsilon, z_batch, t)
...
#                     history.update_with_all_losses(
#                         ts=t_numpy,
#                         losses=unweighted_loss.squeeze().cpu().detach().numpy(),
#                     )
#                 else:
#                     history.update_with_all_losses(
#                         ts=t_numpy,
...
def get_time_prob_path_loss_fn_flow_z_interpolate_epsilons(
    flow,
    flow_name,
    prob_path,
    train,
    ...
    def ctsm_loss_fn(epsilons, epsilon, qx, t):
        targets = prob_path.full_epsilon_target(epsilon, qx, t, factor)

        loss = torch.mean(torch.square(targets - epsilons), dim=(1, 2, 3))
    ...
    def loss_fn(score_model, batch):
        ...
        Returns:
          loss: A scalar that represents the average loss value across the mini-batch.
        """
        score_fn = mutils.get_c_time_epsilons_fn(
            prob_path, score_model, train=train, continuous=continuous
        )
        ...
        if resample_t:
            ...
            y = torch.rand(batch_size, device=device)
            t = 2 * y * z / (1 + torch.sqrt(1 + 4 * y**2 * z**2))
            t = t * (1 - eps) / temp
        else:
            t = torch.rand(batch_size, device=device) * (1 - eps)
...
def get_time_prob_path_loss_fn_flow_zt_z_interpolate_epsilons(
    flow,
    flow_name,
    prob_path,
    train,
    ...
    def loss_fn(score_model, batch):
        ...
        #                 zt.view(batch_size, -1), context=None, rescale=True
        #             )
        #             # px = flow.module.sample(px.view(batch_size, -1), context=None, rescale=True)
        ...
        unweighted_loss = ctsm_loss_fn(score_fn(zt, t), epsilon, z_batch, t)
    ...
    if joint:
        ...
        if not z_space:
            ...
            if z_interpolate:
                ...
                # NOTE: this is the used
                if not conditional:
                    if history:
                        # assert not likelihood_weighting
                        assert likelihood_weighting == "path_var"
                    loss_fn = get_time_sde_loss_fn_flow_z_interpolate(
                        flow,
                        flow_name,
                        sde,
                        train,
                        ...
                        likelihood_weighting=(
                            likelihood_weighting == "sde_var"
                        ),  # compatibility fix
                    )

                elif not epsilons:
                    assert likelihood_weighting == "obj_var"
                    print("You should not use this")
                    raise NotImplementedError
                    # loss_fn = get_time_prob_path_loss_fn_flow_z_interpolate(
                    #     flow,
                    #     flow_name,
                    #     prob_path,
                    #     train,
                    ...
                    loss_fn = get_time_prob_path_loss_fn_flow_z_interpolate_epsilons(
                        flow,
                        flow_name,
                        prob_path,
                        train,
                    ...
                    loss_fn = get_time_prob_path_loss_fn_flow_zt_z_interpolate_epsilons(
                        flow,
                        flow_name,
                        prob_path,
                        train,
