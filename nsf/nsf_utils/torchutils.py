...
def tile(x, n):
    if not nsf_utils.is_positive_int(n):
...
def sum_except_batch(x, num_batch_dims=1):
    ...
    if not nsf_utils.is_nonnegative_int(num_batch_dims):
...
def merge_leading_dims(x, num_dims):
    ...
    if not nsf_utils.is_positive_int(num_dims):
...
def repeat_rows(x, num_reps):
    ...
    if not nsf_utils.is_positive_int(num_reps):
