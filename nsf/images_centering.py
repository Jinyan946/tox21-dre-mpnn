...
@ex.capture
def create_flow(c, h, w, flow_checkpoint, _log, mean_center):
    distribution = distributions.StandardNormal((c * h * w,))
    transform = create_transform(c, h, w)

    if mean_center:
        flow = flows.FlowTRE(transform, distribution)
    else:
        raise NotImplementedError(
            "This module is only defined for mean-centered training!"
        )
        # run vanilla model
...
def train_flow(
    ...
    for step, (batch, _) in enumerate(
        ...
        if multi_gpu:
            ...
            # TODO: modified this
            log_density = flow.log_prob(batch, center=train_mean, context=None)
