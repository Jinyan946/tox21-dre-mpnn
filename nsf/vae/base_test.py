        ...
        for num_samples in [None, 1, 10]:
            with self.subTest(num_samples=num_samples):
                ...
                if num_samples is None:
                    self.assertEqual(
                        encodings.shape, torch.Size([batch_size] + latent_shape)
                    )
                else:
                    self.assertEqual(
                        encodings.shape,
                        torch.Size([batch_size, num_samples] + latent_shape),
                    )
        ...
        for mean in [True, False]:
            for num_samples in [None, 1, 10]:
                with self.subTest(mean=mean, num_samples=num_samples):
                    ...
                    if num_samples is None:
                        self.assertEqual(
                            recons.shape, torch.Size([batch_size] + input_shape)
                        )
                    else:
                        self.assertEqual(
                            recons.shape,
                            torch.Size([batch_size, num_samples] + input_shape),
                        )
