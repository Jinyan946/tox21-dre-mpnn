...
class VariationalAutoencoder(nn.Module):
    ...
    def stochastic_elbo(self, inputs, num_samples=1, kl_multiplier=1, keepdim=False):
        ...
        latents = nsf_utils.merge_leading_dims(latents, num_dims=2)
        log_q_z = nsf_utils.merge_leading_dims(log_q_z, num_dims=2)
        ...
        inputs = nsf_utils.repeat_rows(inputs, num_reps=num_samples)
        ...
        elbo = nsf_utils.split_leading_dim(elbo, [-1, num_samples])
        if keepdim:
            return elbo
        else:
    ...
    def _decode(self, latents, mean):
        if mean:
            ...
            return nsf_utils.merge_leading_dims(samples, num_dims=2)
    ...
    def reconstruct(self, inputs, num_samples=None, mean=False):
        ...
        if num_samples is not None:
            recons = nsf_utils.split_leading_dim(recons, [-1, num_samples])
        return recons
