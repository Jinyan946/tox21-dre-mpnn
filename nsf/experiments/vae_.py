...
from nsf.nde import distributions as distributions_, flows, transforms

parser = argparse.ArgumentParser()

parser.add_argument(
    "--num_runs", type=int, default=1, help="Number of runs with different seeds to do."
)
# data
parser.add_argument(
    "--dataset_name",
...
parser.add_argument(
    "--kl_multiplier_max", type=float, default=1, help="Max value for KL multiplier."
)

# features size
parser.add_argument(
    "--latent_features", type=int, default=32, help="Number of latent features to use."
)
parser.add_argument(
    "--context_features",
    type=int,
...
# flow details
parser.add_argument(
    "--num_flow_steps", type=int, default=5, help="Number of flow steps to use"
)
parser.add_argument(
    "--hidden_features",
    type=int,
...
parser.add_argument(
    "--num_bins", type=int, default=8, help="Number of bins for NSF transforms."
)
parser.add_argument("--tail_bound", type=float, default=3, help="NSF box bound.")
parser.add_argument(
...
parser.add_argument(
    "--dropout_probability_encoder_decoder",
    type=float,
    default=0.0,
...
def run(seed):
    ...
    data_transform = tvtransforms.Compose(
        [tvtransforms.ToTensor(), tvtransforms.Lambda(torch.bernoulli)]
    )

    if args.dataset_name == "mnist":
        dataset = datasets.MNIST(
            root=os.path.join(nsf_utils.get_data_root(), "mnist"),
            train=True,
            download=True,
        ...
        dataset = datasets.FashionMNIST(
            root=os.path.join(nsf_utils.get_data_root(), "fashion-mnist"),
            train=True,
            download=True,
            transform=data_transform,
        )
        ...
        dataset = data_.OmniglotDataset(split="train", transform=data_transform)
        test_dataset = data_.OmniglotDataset(split="test", transform=data_transform)
        ...
        dataset = datasets.EMNIST(
            root=os.path.join(nsf_utils.get_data_root(), "emnist"),
            split="letters",
            train=True,
            transform=data_transform,
            download=True,
    ...
    def create_linear_transform():
        if args.linear_type == "lu":
            ...
            return transforms.SVDLinear(
                args.latent_features, num_householder=4, identity_init=True
            )
    ...
    def get_kl_multiplier(step):
        if args.kl_multiplier_schedule == "constant":
            ...
            multiplier = min(
                step / (args.num_training_steps * args.kl_warmup_fraction), 1.0
            )
            return args.kl_multiplier_initial * (1.0 + multiplier)
    ...
    for step in tbar:
        ...
        if (step + 1) % args.monitor_interval == 0:
            ...
            writer.add_scalar(
                tag="val-elbo", scalar_value=mean_val_elbo, global_step=step
            )

            writer.add_scalar(
                tag="best-val-elbo", scalar_value=best_val_elbo, global_step=step
            )
    ...
    path = os.path.join(
        ...
        "{}-prior-{}-posterior-{}-elbo.npy".format(
            args.dataset_name, args.prior_type, args.approximate_posterior_type
        ),
    )
