...
parser.add_argument(
    "--train_batch_size", type=int, default=64, help="Size of batch used for training."
)
parser.add_argument(
    "--val_frac", type=float, default=1.0, help="Fraction of validation set to use."
)
parser.add_argument(
    "--val_batch_size", type=int, default=512, help="Size of batch used for validation."
)
...
parser.add_argument(
    "--num_flow_steps", type=int, default=10, help="Number of blocks to use in flow."
)
parser.add_argument(
    "--hidden_features",
    type=int,
...
parser.add_argument(
    "--tail_bound", type=float, default=3, help="Box is on [-bound, bound]^2"
)
parser.add_argument(
    "--num_bins",
    type=int,
...
val_loader = data.DataLoader(
    dataset=val_dataset, batch_size=args.val_batch_size, shuffle=True, drop_last=True
)

# test set
test_dataset = data_.load_dataset(args.dataset_name, split="test")
test_loader = data.DataLoader(
    dataset=test_dataset, batch_size=args.val_batch_size, shuffle=False, drop_last=False
)
...
def create_base_transform(i):
    if args.base_transform_type == "affine-coupling":
        return transforms.AffineCouplingTransform(
            mask=nsf_utils.create_alternating_binary_mask(features, even=(i % 2 == 0)),
...
for step in tbar:
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        writer.add_scalar(
            "max-abs-relative-error", max_abs_relative_error, global_step=step
        )
        writer.add_scalar(
            "average-abs-relative-error", average_abs_relative_error, global_step=step
        )
