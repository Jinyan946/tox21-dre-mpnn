...
for batch in grid_loader:
    ...
    log_density_np = np.concatenate((log_density_np, utils.tensor2numpy(log_density)))
...
figure, axes = plt.subplots(1, 1, figsize=(2.5, 2.5))
with torch.no_grad():
    samples = utils.tensor2numpy(flow.sample(num_samples=int(1e6), batch_size=int(1e5)))
axes.hist2d(
    samples[:, 0],
    samples[:, 1],
    range=bounds,
