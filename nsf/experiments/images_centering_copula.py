...
def create_transform_step(
    ...
    rescaling = transforms.AffineTransformv3(
        shape=(784,), scale=1.0 / train_std.view(-1)
    )
...
@ex.capture
def create_flow(c, h, w, train_mean, val_mean, train_std, flow_checkpoint, _log):
    ...
    train_transform, val_transform, transform = create_transform(
        c, h, w, train_mean, val_mean, train_std
    )
...
def train_flow(
    ...
    if cosine_annealing:
        if warmup_fraction == 0.0:
            ...
            # print('reverting back to old scheduler')
        # this seems to do worse
        # how about we try decaying the learning rate after every "epoch"?
        # T_max = 40000 // (50000 // batch_size)
        # print('using {} epochs'.format(T_max))
...
def eval_on_test(batch_size, num_workers, seed, _log, flow_checkpoint):
    ...
    train_mean, test_mean, train_std = get_data_stats(
        train_dataset, test_dataset, train=False
    )

    # TODO: Check logic
    if not os.path.exists(test_stats_path):
        # save these stats
        d = {"train_mean": train_mean, "train_std": train_std, "test_mean": test_mean}
    ...
    train_transform, test_transform, flow = create_flow(
        c, h, w, train_mean, test_mean, train_std
    )
    flow = flow.to(device)
...
def main(seed, _log):
    ...
    train_mean, val_mean, train_std = get_data_stats(train_dataset, val_dataset)

    # save these stats
    d = {"train_mean": train_mean, "val_mean": val_mean, "train_std": train_std}
