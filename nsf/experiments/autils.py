...
def imshow(image, ax):
    image = nsf_utils.tensor2numpy(image.permute(1, 2, 0))
...
def get_dataset_root():
    ...
    try:
        ...
        return "/atlas/u/kechoi/time-score-dre/nsf/"
...
def eval_log_density_transform(
    log_prob_fn, data_loader, transform, device, num_batches=None
):
    """
    same function as before, but applying transformation to the data first
    :param log_prob_fn:
    :param data_loader:
    :param transform:
    :param num_batches:
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            # apply val transform
            batch, logabsdet = transform(batch)
            log_prob = log_prob_fn(batch)
...
class Conv2dSameSize(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, bias=True):
        ...
        super().__init__(
            in_channels, out_channels, kernel_size, bias=bias, padding=same_padding
        )
