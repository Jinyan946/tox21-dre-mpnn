...
# data
parser.add_argument(
    "--dataset_name", type=str, default="diamond", help="Name of dataset to use."
)
parser.add_argument(
    "--n_data_points",
    default=int(1e6),
...
parser.add_argument(
    "--batch_size", type=int, default=512, help="Size of batch used for training."
)
parser.add_argument(
    "--num_workers", type=int, default=0, help="Number of workers used in data loaders."
)

# model
parser.add_argument("--num_bins", type=int, default=64, help="Number of bins to use ")

# optimization
parser.add_argument("--learning_rate", default=5e-4, help="Learning rate for Adam.")
parser.add_argument(
    "--num_training_steps", default=int(5e5), help="Number of total training steps."
)
parser.add_argument(
    "--grad_norm_clip_value",
    type=float,
    default=5.0,
...
parser.add_argument(
    "--save_interval", default=10000, help="Interval in steps at which to save model."
)

# reproducibility
parser.add_argument(
    "--seed", default=1638128, help="Random seed for PyTorch and NumPy."
)
...
grid_dataset = data_.TestGridDataset(
    num_points_per_axis=num_points_per_axis, bounds=bounds
)
grid_loader = data.DataLoader(dataset=grid_dataset, batch_size=1000, drop_last=False)
dim = 2
...
def create_base_transform(i):
    if args.base_transform_type == "affine":
        return transforms.AffineCouplingTransform(
            mask=nsf_utils.create_alternating_binary_mask(
...
n_params = nsf_utils.get_num_parameters(flow)
...
for step in tbar:
    ...
    if (step + 1) % args.visualize_interval == 0:
        ...
        cmap = cm.magma
        axes[0].hist2d(
            utils.tensor2numpy(train_dataset.data[:, 0]),
        ...
        path = os.path.join(
            cutils.get_output_root(), "{}.png".format(args.dataset_name)
        )
    ...
    if (step + 1) % args.save_interval == 0:
        path = os.path.join(
            cutils.get_checkpoint_root(), "{}.t".format(args.dataset_name)
        )
        torch.save(flow.state_dict(), path)

path = os.path.join(
    cutils.get_checkpoint_root(), "{}-{}.t".format(args.dataset_name, timestamp)
)
torch.save(flow.state_dict(), path)
