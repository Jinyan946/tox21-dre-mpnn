...
class ConvNet(nn.Module):
    """
    more sophisticated (?) convnet based on TRE paper
    """
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, dropout=0.0):
        ...
        self.hidden_channels = hidden_channels
        self.net = nn.Sequential(
            # not adding edge bias, whatever that means
            # no bias
            Conv2dSameSize(in_channels, hidden_channels, kernel_size=3),
            transforms.ActNorm(hidden_channels),
            nn.ReLU(),
            nn.Dropout(dropout),
            # no bias
            Conv2dSameSize(hidden_channels, hidden_channels, kernel_size=1),
            ...
            nn.Dropout(dropout),
            # no actnorm here
            # Conv2dSameSize(hidden_channels, out_channels, kernel_size=3, bias=True),
        )
        self.final_conv = Conv2dSameSize(
            hidden_channels, out_channels, kernel_size=3, bias=True
        )
        self.logs = nn.Parameter(torch.zeros(1, out_channels))

    def forward(self, inputs, context=None):
        out = self.net.forward(inputs)
        out = self.final_conv(out)
        # TODO: beware of shape?
        import pdb

        pdb.set_trace()
        out *= torch.exp(1.0 * self.logs)  # not sure why...
        return out
...
def create_transform_step(
    ...
    if use_resnet:
        ...
        # TODO: why was this zeroed out?
        # if dropout_prob != 0.:
        #     raise ValueError()
        def create_convnet(in_channels, out_channels):
            return ConvNet(
                in_channels, hidden_channels, out_channels, dropout=dropout_prob
            )

    mask = nsf_utils.create_mid_split_binary_mask(num_channels)
...
@ex.capture
def create_flow(c, h, w, train_mean, val_mean, flow_checkpoint, _log):
...
def train_flow(
    ...
    for step, (batch, _) in enumerate(
        ...
        # Register with hook
        flow._transform._transforms[1]._transforms[-1]._transforms[-1]._transforms[
            -1
        ]._scale.register_hook(get_zero_grad_hook(mask))
        ...
        if scheduler is not None:
            ...
            # summary['learning_rate'] = scheduler.get_lr()[0]
        ...
        #     wandb.run.summary["best_val_log_prob"] = best_val_log_prob
        #     # summary['best_val_log_prob'] = best_val_log_prob
        # summary = dict(loss=loss.item(), step=step)
        # wandb.log(summary)
        ...
        if step % intervals["sample"] == 0:
            ...
            # wandb.log({"samples": [wandb.Image(i) for i in samples[0:64]]})
        ...
        if step > 0 and step % intervals["eval"] == 0 and (val_loader is not None):
            ...
            # summary = dict(val_log_prob=val_log_prob, step=step)
            # wandb.log(summary)
...
def eval_on_test(batch_size, num_workers, seed, _log):
    ...
    #     'test_mean': test_mean
    # }
    # with open(os.path.join(fso.dir, 'test_data_means.p'), 'wb') as fp:
    #     pickle.dump(d, fp)
    train_mean = train_mean.to(device)
    test_mean = test_mean.to(device)

    # train_transform, val_transform, flow = create_flow(c, h, w)
    train_transform, test_transform, flow = create_flow(c, h, w, train_mean, test_mean)
    flow = flow.to(device)
...
def main(seed, _log):
    ...
    # save these stats
    d = {"train_mean": train_mean, "val_mean": val_mean}
    ...
    # train_transform, val_transform, flow = create_flow(c, h, w)
    train_transform, val_transform, flow = create_flow(c, h, w, train_mean, val_mean)
