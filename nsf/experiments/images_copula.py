...
def create_transform_step(
    ...
    step_transforms.append(cholesky)
    ...
    transform_level = transforms.CompositeTransform(
        [affine_transform1] + [coupling_layer] + [affine_transform2]
    )
    step_transforms.append(transform_level)

    # TRE also adds a rescaling factor here at the end, scaling by train_data's featurewise stdev
...
def create_transform(
    ...
    # TODO: if we invert this, we need to flip the order
...
#
#     new_ckpt = {}
#     var_names = ['_transform._transforms.0._transforms.0._shift', '_transform._transforms.0._transforms.0._scale', '_transform._transforms.0._transforms.1._shift', '_transform._transforms.0._transforms.1._scale', '_transform._transforms.1._transforms.0._transforms.1._transforms.0._shift', '_transform._transforms.1._transforms.0._transforms.1._transforms.0._scale', '_transform._transforms.1._transforms.0._transforms.1._transforms.1.unnormalized_widths', '_transform._transforms.1._transforms.0._transforms.1._transforms.1.unnormalized_heights', '_transform._transforms.1._transforms.0._transforms.1._transforms.1.unnormalized_derivatives', '_transform._transforms.1._transforms.0._transforms.1._transforms.2._shift', '_transform._transforms.1._transforms.0._transforms.1._transforms.2._scale', '_transform._transforms.1._transforms.0._transforms.1._transforms.3._shift', '_transform._transforms.1._transforms.0._transforms.1._transforms.3._scale']
#     if flow_checkpoint is not None:
#         flow_ckpt = torch.load(flow_checkpoint)
#
#         # first get the preprocessing stuff out of the way
#         for i in range(4):
#             new_ckpt[var_names[i]] = flow_ckpt[var_names[i]]
#
#         import pickle
#         with open('/atlas/u/kechoi/time-score-dre/nsf/tre_norescale_ckpt.p', 'rb') as fp:
#             tf_ckpt = pickle.load(fp)
#         tf_ckpt_values = [x for x in reversed(tf_ckpt.values())]
#
#         for j in range(4, len(var_names)):
#             print(var_names[j])
#             item = flow_ckpt[var_names[j]]
#             print(item.size(), tf_ckpt_values[j-4].shape)
#             if j == 6:
#                 new_ckpt[var_names[j]] = torch.from_numpy(
#                     tf_ckpt_values[4]).view(item.size()).float()
#             elif j == 7:
#                 new_ckpt[var_names[j]] = torch.from_numpy(
#                     tf_ckpt_values[3]).view(item.size()).float()
#             elif j == 8:
#                 new_ckpt[var_names[j]] = torch.from_numpy(
#                     tf_ckpt_values[2]).view(item.size()).float()
#             else:
#                 try:
#                     new_ckpt[var_names[j]] = torch.from_numpy(tf_ckpt_values[j-4]).view(item.size()).float()
#                 except:
#                     new_ckpt[var_names[j]] = torch.from_numpy(
#                         tf_ckpt_values[j - 4]).float()
#
#         flow.load_state_dict(new_ckpt)
#         _log.info('Flow state loaded from {}'.format(flow_checkpoint))
#
#     return flow
...
def train_flow(
    ...
    # Register with hook
    # flow._transform._transforms[1]._transforms[-1]._transforms[-1]._transforms[-1]._scale.register_hook(get_zero_grad_hook(mask))

    # not reversed
    # flow._transform._transforms[1]._transforms[-1]._transforms[-1]._transforms[0]._scale.register_hook(get_zero_grad_hook(mask))
...
def eval_on_test(batch_size, num_workers, seed, _log):
    ...
    mean, err = autils.eval_log_density_2(
        log_prob_fn=log_prob_fn, data_loader=test_loader, c=c, h=h, w=w
    )
    print("Test log probability (bits/dim): {:.4f} +/- {:.4f}".format(mean, err))
...
def main(seed, _log):
    ...
    train_flow(flow, train_dataset, val_dataset, test_dataset, (c, h, w), device)
