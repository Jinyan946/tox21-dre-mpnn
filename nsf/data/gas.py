...
def load_gas():
    ...
    return load_data_and_clean_and_split(
        file=os.path.join(nsf_utils.get_data_root(), "gas", "ethylene_CO.pickle")
    )
...
def save_splits():
    ...
    for split in splits:
        name, data = split
        file = os.path.join(nsf_utils.get_data_root(), "gas", "{}.npy".format(name))
        np.save(file, data)


class GasDataset(Dataset):
    def __init__(self, split="train", frac=None):
        path = os.path.join(nsf_utils.get_data_root(), "gas", "{}.npy".format(split))
