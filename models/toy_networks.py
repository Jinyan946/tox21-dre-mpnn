...
@utils.register_model(name="toy_full_time_scorenet")
class TimeScoreNetwork(nn.Module):
    """
    Simple MLP-based score network (for toy gaussian problems)
    """
...
class MVNParamScoreNetwork(nn.Module):
    ...
    def forward(self, x, t):
        ...
        term1 = t[:, :, 0, 0] * torch.einsum(
            "bii->b", (theta_mat @ new_cov_inv).squeeze()
        ).unsqueeze(1)
        term2 = (
            t
            * (
                x.view(-1, 1, 1, self.dim)
                @ new_cov_inv
                @ theta_mat
                @ new_cov_inv
                @ x.view(-1, 1, 1, self.dim).permute(0, 1, 3, 2)
            )
        )[:, :, 0, 0]
        out_t = -term1 + term2
...
@utils.register_model(name="toy_full_param_mvn_mi")
class FullMVNParamScoreNetwork(nn.Module):
    """
    learning the parameterized score network for multivariate gaussians (high dimensional gaussians experiment for mutual information estimation)
    """
    ...
    def forward_full(self, x, t):
        ...
        temp = 1 - t**2
        post_cov = temp * new_cov_inv @ (theta_mat + id_mat)
        post_mean = (t / temp).squeeze(-1) * torch.einsum("nijk,nk->nij", post_cov, x)
        x = x[:, None]
        # print(post_mean.shape, post_cov.shape, x.shape, t.shape, temp.shape)

        # test_t = t.view(-1, 1)
        # test_temp = temp.view(-1, 1)
        # test_out_t = (
        #     self.dim * test_t * test_temp
        #     - test_t
        #     * (
        #         torch.sum(torch.square(post_mean), dim=-1)
        #         + torch.einsum("ijkk", post_cov)
        #     )
        #     - test_t * torch.sum(torch.square(x), dim=-1)
        #     + (test_t**2 + 1) * torch.sum(x * post_mean, dim=-1)
        # ) / test_temp**2
        ...
        out_t = (
            t * temp
            - t * (torch.square(post_mean) + torch.diagonal(post_cov, dim1=-2, dim2=-1))
            - t * torch.square(x)
            + (t**2 + 1) * x * post_mean
        ) / temp**2

        if self.config.model.type == "time":
            return out_t.squeeze()
        else:
            out_x = -(x.view(-1, 1, 1, self.dim) @ new_cov_inv).squeeze()
            return out_x, out_t.squeeze()
