...
def dilated_conv3x3(in_planes, out_planes, dilation=1, stride=1, bias=True):
    conv = nn.Conv2d(
        in_planes,
        out_planes,
        kernel_size=3,
...
# NOTE: that this is from a previous version of NCSNv2!!!!!
    ...
    def __init__(
        ...
        if resample == "down":
            if dilation is not None:
                ...
                # self.normalize2 = normalization(input_dim)
                self.normalize2 = normalization(
                    num_groups=min(output_dim, 32), num_channels=input_dim, eps=1e-6
                )
            ...
            if dilation is not None:
                ...
                # self.normalize2 = normalization(output_dim)
                self.normalize2 = normalization(
                    num_groups=min(output_dim, 32), num_channels=output_dim, eps=1e-6
                )
