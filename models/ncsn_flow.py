...
import yaml
import logging
from models.ncsn_unet import SinusoidalPosEmb, GaussianFourierProjection, Dense
...
def load_pretrained_flow(config, test=False):
    ...
    if name in ["mintnet", "nice"]:
        ...
        if not test:
            ...
            with open(
                os.path.join(top_path, "flow_ckpts", "test_data_means.p"), "rb"
            ) as fp:
                data_stats = pickle.load(fp)
        ...
        # net = flows.Flow(transform, distribution)
        net = flows.FlowDataTransform(
            transform, distribution, train_transform, val_transform
        )
        ...
        xs = {
            "_train_transform." + k: v for k, v in train_transform.state_dict().items()
        }
        ...
        # load data stats, no need to load a separate checkpoint
        with open(
            os.path.join(top_path, "flow_ckpts", "test_data_stats.p"), "rb"
        ) as fp:
            data_stats = pickle.load(fp)
        ...
        base_dist = Independent(
            Normal(
                torch.zeros((784,), device=device),
                torch.ones((784,), device=device),
            ),
            reinterpreted_batch_ndims=1,
        )
        transform = ReshapeTransform((784,), (1, 28, 28))
        net = TransformedDistribution(
            base_dist,
            [
                transform,
            ],
...
class NCSNMLPv2(nn.Module):
    ...
    def __init__(self, config, embed_dim=256):
        ...
        self.dense = nn.ModuleList(
            [
                Dense(self.h_dim, self.h_dim)
                for _ in range(config.model.n_hidden_layers + 1)
            ]
        )
