...
flags.DEFINE_string("project", "dre", "the wandb project for storing the runs")
flags.DEFINE_string("doc", None, "exp_name")
flags.DEFINE_bool("toy", False, "whether to run toy experiment")
flags.DEFINE_bool(
    "flow",
    False,
    "whether to encode the data into latent space using a pre-trained flow",
)
...
def main(argv):
    ...
    if FLAGS.mode == "train":
        ...
        with open(os.path.join(FLAGS.workdir, "config.txt"), "w") as f:
            ...
            if not FLAGS.toy:
                if not FLAGS.flow:
                    ...
                    logger.info(
                        "Leveraging pre-trained flow model for score estimation!"
                    )
        ...
        if FLAGS.flow:
            ...
            if "rq_nsf" in FLAGS.config.training.z_space_model:
                run_lib_rqnsf_flow.evaluate(
                    FLAGS.config, FLAGS.workdir, FLAGS.eval_folder
                )
            else:
                import run_lib_flow
