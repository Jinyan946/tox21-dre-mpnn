...
from models import ncsn_unet, ncsnpp
import losses
import interp_losses
import sampling
...
# from torch.nsf_utils import tensorboard
...
def train(config, workdir):
    ...
    if not config.training.z_space:
        ...
        logging.info(
            "Using z-space encodings from pretrained MintNet for training time score model!"
        )
        train_ds, eval_ds = datasets.get_encoded_dataset(
            config, uniform_dequantization=config.data.uniform_dequantization
        )
    ...
    if config.training.sde.lower() == "vpsde":
        ...
        sde = sde_lib.InterpXt(
            t_min=config.training.eps, t_max=1.0, N=config.model.num_scales
        )
        sampling_eps = 1e-5
    elif config.training.sde.lower() == "fancy_interpxt":
        sde = sde_lib.FancyInterpXt(
            t_min=config.training.eps, t_max=1.0, N=config.model.num_scales
        )
        sampling_eps = 1e-5
    else:
    ...
    # if config.training.sde.lower() in ['interpxt', 'fancy_interpxt']:
    #   logging.info('using loss function from special linear interpolation scheme!')
    ...
    if config.training.dre_bpd_v2:
        if config.training.sde.lower() in ["interpxt", "fancy_interpxt"]:
            density_ratio_fn_v2 = density_ratios.get_interp_v2_density_ratio_fn(
                sde, inverse_scaler, eps=train_eps
            )
        else:
            density_ratio_fn_v2 = density_ratios.get_v2_density_ratio_fn(
                sde, inverse_scaler, eps=train_eps
            )
    if config.training.from_xscore:
        estimated_density_ratio_fn = density_ratios.get_density_ratios_from_data_scores(
            sde, inverse_scaler, eps=train_eps, rtol=1e-3, atol=1e-6
        )  # default values
    ...
    for step in range(initial_step, num_train_steps + 1):
        ...
        else:  # TODO: only pytorch, this is for z-space training
            try:
        ...
        if step % config.training.log_freq == 0:
            logging.info(
                "step: %d, training_loss: %.5e" % (step, summary["loss"].item())
            )
        ...
        if step % config.training.eval_freq == 0:
            ...
            # dequantize this batch for ratio computation
            dre_eval_batch = copy.deepcopy(eval_batch)
            dre_eval_batch = (
                torch.rand_like(dre_eval_batch) + dre_eval_batch * 255.0
            ) / 256.0
            # normalize both eval batches
            dre_eval_batch = scaler(dre_eval_batch)
            eval_batch = scaler(eval_batch)
            eval_loss = eval_step_fn(state, eval_batch)
            summary = dict(test_loss=eval_loss["loss"], step=step)
            logging.info("step: %d, eval_loss: %.5e" % (step, eval_loss["loss"].item()))
            ...
            if step > 100 and step % config.training.ratio_freq == 0:
                if config.eval.enable_bpd:
                    ...
                    if config.training.pf_ode_bpd:
                        ...
                        summary["test_ode_bpds"] = bpd.mean()
                        logging.info(
                            "step: %d, eval_bpd (PF ODE): %.5f" % (step, bpd.mean())
                        )
                    if config.training.dre_bpd:
                        dre_bpd = density_ratio_fn(score_model, dre_eval_batch)[0]
                        dre_bpd = dre_bpd.reshape(-1)
                        summary["test_dre_bpds"] = dre_bpd.mean()
                        logging.info(
                            "step: %d, eval_dre_bpd (time scores): %.5f"
                            % (step, dre_bpd.mean())
                        )
                    if config.training.dre_bpd_v2:
                        ...
                        logging.info(
                            "step: %d, eval_dre_bpd_v2 (joint scores): %.5f"
                            % (step, dre_bpd_v2.mean())
                        )
                    if config.training.from_xscore:
                        est_bpd = estimated_density_ratio_fn(
                            score_model, dre_eval_batch
                        )[0]
                        est_bpd = est_bpd.reshape(-1)
                        summary["est_bpds"] = est_bpd.mean()
                        logging.info(
                            "step: %d, eval_bpd via fokker-planck: %.5f"
                            % (step, est_bpd.mean())
                        )
...
def evaluate(config, workdir, eval_folder="eval"):
    ...
    # TODO: incorporate AIS evaluation here!
    if config.eval.enable_bpd:
        likelihood_fn = likelihood.get_likelihood_fn(sde, inverse_scaler)
        if config.training.algo != "baseline":
            if config.training.sde.lower() in ["interpxt", "fancy_interpxt"]:
                ...
                if config.evaluate.ais:
                    density_ratio_fn = density_ratios.get_density_ratio_fn_ais(
                        sde, config.evaluate.ais_steps, inverse_scaler
                    )
                else:
                    density_ratio_fn = density_ratios.get_density_ratio_fn(
                        sde, inverse_scaler
            ...
            density_ratio_fn = density_ratios.get_density_ratios_from_data_scores(
                sde, inverse_scaler, rtol=1e-3, atol=1e-3
            )
        if config.training.dre_bpd_v2:
            if config.training.sde.lower() in ["interpxt", "fancy_interpxt"]:
                density_ratio_fn_v2 = density_ratios.get_interp_v2_density_ratio_fn(
        ...
        if config.eval.enable_bpd:
            ...
            for repeat in range(bpd_num_repeats):
                ...
                for batch_id in range(len(ds_bpd)):
                    ...
                    eval_batch = eval_batch.permute(0, 3, 1, 2)
                    eval_batch = scaler(eval_batch)
                    # this is from the probability flow ode
                    # bpd = likelihood_fn(score_model, eval_batch)[0]
                    # bpd = bpd.detach().cpu().numpy().reshape(-1)
                    bpd = density_ratio_fn(score_model, eval_batch)[0]
                    bpd = bpd.reshape(-1)
                    bpds.extend(bpd)
                    logging.info(
