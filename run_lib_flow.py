...
def train(config, workdir):
    ...
    if config.training.sde.lower() == "vpsde":
        ...
        assert flow is not None
        sde = sde_lib.FlowInterpXt(
            flow, t_min=config.training.eps, t_max=1.0, N=config.model.num_scales
        )
    else:
    ...
    if config.training.algo != "baseline":
        if config.training.sde.lower() in ["interpxt", "flow_interpxt"]:
            ...
            if not config.training.z_space:
                ...
                if z_interpolate:
                    density_ratio_fn = (
                        density_ratios.get_z_interp_density_ratio_fn_flow(
                            sde, inverse_scaler, mlp=mlp
                        )
                    )
                else:
        ...
        if step % config.training.eval_freq == 0:
            if not config.training.z_space:
                ...
                if invert_flow or z_interpolate:  # p(x) = flow
                    ...
                    # get log-det-logit
                    log_det_logit = (
                        F.softplus(-eval_batch).sum()
                        + F.softplus(eval_batch).sum()
                        + np.prod(eval_batch.shape)
                        * np.log(1 - 2 * config.data.lambda_logit)
                    )
            ...
            if config.training.sde.lower() in ["interpxt", "flow_interpxt"]:
                ...
                if not config.eval.ais:
                    ...
                    density_ratio_fn = density_ratios.get_density_ratio_fn_ais(
                        sde, inverse_scaler, n_ais_steps, n_ais_samples, fancy=fancy
                    )
        else:
            ...
            for repeat in range(bpd_num_repeats):
                ...
                for batch_id in range(len(ds_bpd)):
                    ...
                    bpd = density_ratio_fn(
                        flow=flow, score_model=score_model, x=eval_batch
                    )[0]
