        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                if "noise" in flow_name or "copula" in flow_name:
                    data, logabsdet = flow.module.transform_to_noise(
                        data, context=None, transform=True, train=False, logdet=True
                    )
                else:
                    data, logabsdet = flow.module.transform_to_noise(
                        data, context=None, logdet=True
                    )
            ...
            prior_logp = (
                ...
                - torch.sum(z.view(n, -1) ** 2, dim=-1) / 2.0
            )
            prior_logp = prior_logp + N * np.log(256)
            prior_logp = prior_logp - N * np.log(2)
            #
            bpd = -(
                prior_logp.to(delta_logp.device)
                + delta_logp
                + logabsdet.to(delta_logp.device)
            ) / (np.log(2) * N)
            offset = 7.0  # 8.
            bpd = bpd + offset
