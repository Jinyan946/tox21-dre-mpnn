...
def elu_derivative(x, slope=1.0):
    ...
    x = torch.min(x, torch.ones_like(x) * 70.0)
...
class DataParallelWithSampling(nn.DataParallel):
    ...
    def parallel_apply_sampling(self, replicas, inputs, kwargs):
        return parallel_apply_sampling(
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.bias1 = nn.Parameter(torch.zeros(input_dim * latent_dim))
        ...
        self.weight2 = nn.Parameter(
            torch.randn(
                input_dim * latent_dim, input_dim * latent_dim, kernel2, kernel2
            )
            * 1e-5
        )
        self.bias2 = nn.Parameter(torch.zeros(input_dim * latent_dim))
        ...
        self.bias3 = nn.Parameter(torch.zeros(input_dim))
        if not init_zero:
        ...
        generate_masks(
            ...
            latent_dim,
            type,
            config.model.rgb_last,
        )

        self.mask1 = nn.Parameter(torch.from_numpy(self.mask1), requires_grad=False)
        self.center_mask1 = nn.Parameter(
            torch.from_numpy(self.center_mask1), requires_grad=False
        )
        self.mask2 = nn.Parameter(torch.from_numpy(self.mask2), requires_grad=False)
        self.center_mask2 = nn.Parameter(
            torch.from_numpy(self.center_mask2), requires_grad=False
        )
        self.mask3 = nn.Parameter(torch.from_numpy(self.mask3), requires_grad=False)
        self.center_mask3 = nn.Parameter(
            torch.from_numpy(self.center_mask3), requires_grad=False
        )
    ...
    def forward(self, x):
        ...
        kernel_mid_y, kernel_mid_x = (
            masked_weight1.shape[-2] // 2,
            masked_weight1.shape[-1] // 2,
        )
        ...
        diag1 = (
            self.non_linearity_derivative(latent_output).view(
                x.shape[0], self.latent_dim, self.input_dim, x.shape[-2], x.shape[-1]
            )
            * diag1[None, :, :, None, None]
        )  # shape: B x latent_dim x input_dim x img_shape x img_shape

        latent_output = self.non_linearity(latent_output)

        center1 = (
            masked_weight1 * self.center_mask1
        )  # shape: latent_dim.input_dim x input_dim x kernel x kernel
        center3 = (
            masked_weight3 * self.center_mask3
        )  # shape: input_dim x latent_dim.input_dim x kernel x kernel

        # shape: 1 x latent_dim x input_dim x input_dim x kernel x kernel
        center1 = center1.view(
            self.latent_dim,
            self.input_dim,
            self.input_dim,
            center1.shape[-2],
        ...
        center2 = sign_prods[..., self.kernel3 // 2, self.kernel1 // 2].unsqueeze(
            -1
        ).unsqueeze(-1) * torch.abs(center2)
        center2 = center2.permute(0, 2, 1, 3, 4, 5).contiguous().view_as(self.weight2)

        masked_weight2 = (
            center2 * self.center_mask2 + self.weight2 * (1.0 - self.center_mask2)
        ) * self.mask2
        ...
        kernel_mid_y, kernel_mid_x = (
            masked_weight2.shape[-2] // 2,
            masked_weight2.shape[-1] // 2,
        )
        diag2 = masked_weight2[..., kernel_mid_y, kernel_mid_x].view(
            self.latent_dim, self.input_dim, self.latent_dim, self.input_dim
        )
        diag2 = torch.diagonal(
            diag2.permute(0, 2, 1, 3), dim1=-2, dim2=-1
        )  # shape: latent_dim x latent_dim x input_dim
        diag2 = diag2[
            None, :, :, :, None, None
        ]  # shape: 1 x latent_dim x latent_dim x input_dim x 1 x 1

        diag2 = torch.sum(
            diag2 * diag1.unsqueeze(1), dim=2
        )  # shape: B x latent_dim x input_dim x img_shape x img_shape
        ...
        kernel_mid_y, kernel_mid_x = (
            masked_weight3.shape[-2] // 2,
            masked_weight3.shape[-1] // 2,
        )
        diag3 = masked_weight3[..., kernel_mid_y, kernel_mid_x].view(
            self.input_dim, self.latent_dim, self.input_dim
        )
        diag3 = torch.diagonal(
            diag3.permute(1, 0, 2), dim1=-2, dim2=-1
        )  # shape: latent_dim x input_dim
        diag3 = (
            latent_output_derivative.view(
                x.shape[0], self.latent_dim, self.input_dim, x.shape[-2], x.shape[-1]
            )
    ...
    def sampling(self, z):
        with torch.no_grad():
            ...
            center2 = (
                center2.permute(0, 2, 1, 3, 4, 5).contiguous().view_as(self.weight2)
            )
            masked_weight2 = (
                center2 * self.center_mask2 + self.weight2 * (1.0 - self.center_mask2)
            ) * self.mask2
            ...
            diag2_share = masked_weight2[..., kernel_mid_y, kernel_mid_x].view(
                self.latent_dim, self.input_dim, self.latent_dim, self.input_dim
            )
            diag2_share = torch.diagonal(
                diag2_share.permute(0, 2, 1, 3), dim1=-2, dim2=-1
            )  # shape: latent_dim x latent_dim x input_dim
            diag2_share = diag2_share[
                None, :, :, :, None, None
            ]  # shape: 1 x latent_dim x latent_dim x input_dim x 1 x 1
            ...
            diag3_share = masked_weight3[..., kernel_mid_y, kernel_mid_x].view(
                self.input_dim, self.latent_dim, self.input_dim
            )
            diag3_share = torch.diagonal(
                diag3_share.permute(1, 0, 2), dim1=-2, dim2=-1
            )[None, :, :, None, None]
            ...
            def value_and_grad(x):
                ...
                diag2 = torch.sum(
                    diag2_share * diag1.unsqueeze(1), dim=2
                )  # shape: B x latent_dim x input_dim x img_shape x img_shape
                latent_output_derivative = self.non_linearity_derivative(latent_output)
                latent_output = self.non_linearity(latent_output)
                latent_output = F.conv2d(
...
class SpaceToDepth(nn.Module):
    ...
    def sampling(self, z):
        with torch.no_grad():
            ...
            t_1 = output.reshape(
                batch_size, d_height, d_width, self.block_size_sq, s_depth
            )
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        subsampling_anchors = [
            subsampling_gap * (i + 1) for i in range(config.model.n_subsampling)
        ]
