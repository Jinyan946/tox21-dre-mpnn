...
class BasicBlock(nn.Module):
    ...
    def init_conv_weight(self, weight):
        init.kaiming_uniform_(weight, math.sqrt(5.0))
...
class Net(nn.Module):
    ...
    def _make_layer(
        self, shape, block_num, latent_dim, input_dim, init_zero, batch_norm=False
    ):
        layers = []
    ...
    def forward(self, x):
        ...
        if not self.config.model.pad_zero:
            ...
            padding = torch.zeros(
                x.shape[0], 16 - x.shape[1], x.shape[-2], x.shape[-1], device=x.device
            )
